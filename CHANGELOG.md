...
## [0.0.16] - 2025-08-05
### Added
- Added Advanced section uninstall data management option - admins can choose to keep or remove all plugin data when uninstalling
- Added comprehensive uninstall.php script that honors admin data retention preferences
- Added detailed UI explanations and warnings for uninstall data handling options
- Added proper radio button interface for uninstall data retention setting
- Implemented manual inclusion of league/html-to-markdown library for distribution
- Added local autoloader for html-to-markdown library

### Fixed
- Fixed foreign key constraint errors during plugin activation/deactivation
- Fixed table drop order in reset_database() and uninstall logic (child tables before parent tables)
- Fixed database error "Cannot drop table 'wp_gptpg_unique_posts' referenced by a foreign key constraint"
- Fixed fatal error when installing plugin from GitHub ZIP (missing vendor directory)
- Fixed incomplete uninstall cleanup - ensure gptpg_db_version option is deleted
- Removed Composer dependency requirement - plugin now works out-of-the-box
...
- Fixed missing admin notifications by adding proper settings_errors() call to settings page
...
## [0.0.14] - 2025-07-31
### Fixed
- Fixed nonce validation errors across all AJAX steps (Step 1-4)
- Fixed GitHub snippet link extraction to show only relevant URLs in Step 3
- Fixed gist embed URL decoding to extract actual GitHub URLs from target parameter
- Fixed GitHub organization name from 'ipokkel' to 'strangerstudios' in all extraction patterns
- Fixed Step 2 data flow to preserve GitHub links detected in Step 1
- Fixed fallback to potential snippet URL generation when actual links are available

### Changed
- Updated debug mode from 'debug' to 'production' for cleaner operation
- Improved GitHub URL extraction logic to handle gist embed URLs properly
- Enhanced Step 1→2→3 data flow to maintain GitHub link consistency

### Removed
- Removed debug logging to WordPress debug.log while preserving internal plugin debugging
